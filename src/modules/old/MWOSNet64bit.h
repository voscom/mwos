#ifndef MWOS3_MWOSNET64BIT_H
#define MWOS3_MWOSNET64BIT_H

#include "../../core/adlib/MWTimeout.h"
#include "../../core/MWBus_CAN.h"

#ifndef timeoutSendCAN_mks
...
public:
    ...
    bool Read() {
        ...
        if (size>0) { // принимаем только по 8 байт!
            ...
            if (module!=NULL) {
                MWOSNetReceiverFields receiverFields;
                receiverFields.param_id=frameDat.param_id;
                receiverFields.array_index=frameDat.index;
                receiverFields.receiveValue=frameDat.value;
                module->onReceiveCmd((MWOSProtocolCommand) frameDat.cmd,&receiverFields);
            }
            return true;
        }
        return false;
    ...
    virtual void onEvent(MWOSModeEvent modeEvent) {
        if (modeEvent!=EVENT_UPDATE) return; // Вызывается каждый тик операционной системы
        if (Read()) {
