 ...
 * Неодходимо добавить в platformio.ini
 lib_deps =
  ...
  Можно оставить Ethernet2, но тогда в момент подключения к сети, MWOS будет блокироваться на пару секунд
  lib_deps =
        Ethernet2
 ...
 * или если не приходят данные от сервера больше MW_ReceiveTimeout_DSec [600 дСек = 60 сек]
 *
...
#endif
#ifndef MWOS_NET_MISO
#define MWOS_NET_MISO MISO
#endif
#ifndef MWOS_NET_MOSI
#define MWOS_NET_MOSI MOSI
#endif
#ifndef MWOS_NET_SCK
#define MWOS_NET_SCK SCK
#endif
...
#pragma pack(pop)
    ...
    MWOS_PARAM(10, pinsW5500, MWOS_PIN_INT_PTYPE, mwos_param_pin+mwos_param_readonly, MWOS_STORAGE_NO, 4);
    ...
    virtual int64_t getValue(MWOSParam * param, int16_t arrayIndex) {
        if (param==&p_pinsW5500) {
            if (arrayIndex==0) return MWOS_NET_MOSI;
            if (arrayIndex==1) return MWOS_NET_MISO;
            if (arrayIndex==2) return MWOS_NET_SCK;
    ...
    virtual void connectNet() {// подключение и инициализация сети
        ...
        uint16_t id_mac=(id_cn[0] << 8) +id_cn[1];
        if (id_mac==0) id_mac=random(1,UINT16_MAX);
        mac[5] = id_mac & 0xff;
        mac[4] = (id_mac >> 8) & 0xff;
        MW_LOG_MODULE(this); MW_LOG(F("MAC")); MW_LOG_BYTES((unsigned char *) &mac,6);
        MW_LOG(F(", MOSI:")); MW_LOG(MWOS_NET_MOSI); MW_LOG(F(", MISO:")); MW_LOG(MWOS_NET_MISO);
        MW_LOG(F(", SCK:")); MW_LOG(MWOS_NET_SCK); MW_LOG(F(", SS:")); MW_LOG_LN(MWOS_NET_SS);
        Ethernet.init(MWOS_NET_SS);
        if (IsStaticIP()) {
            Ethernet.begin(mac, getStaticIP(), getStaticDNS(), getStaticGateway(), getStaticSubnet());
        } else
            Ethernet.begin(mac);
        MW_LOG_MODULE(this); MW_LOG(F("Ethernet ip: ")); MW_LOG(Ethernet.localIP().toString()); MW_LOG(F(", subnet: ")); MW_LOG(Ethernet.subnetMask().toString());
        MW_LOG(F(", gateway: ")); MW_LOG(Ethernet.gatewayIP().toString()); MW_LOG(F(", DNS: ")); MW_LOG_LN(Ethernet.dnsServerIP().toString());
        MWOSNetIP::connectNet();
    }

    virtual uint8_t isConnectedNet() { // есть подключение к сети?
        Ethernet.maintain();
        return 1;
    }
