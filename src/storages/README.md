...
- #### Хранилище 0, типа MWOSStorageEEPROM
    Бинарное хранилище в EEPROM (или эмуляторе EEPROM в FLASH). Есть почти на всех контроллерах.
    Обычно имеет размер от 2Kb до 4Kb и ограниченное количество перезаписей.
...
Стандартно MWOS3 для ESP32 содержит 3 хранилища (два стандартных и одно дополнительное в NVS):

- #### Хранилище 2, типа MWOSStorageEspNVS
  Универсальное динамическое бинарное хранилище в энергонезависимой памяти. Поддерживаются только контроллеры ESP. 
- Позволяет хранить большие объемы с ограниченным количеством перезаписей. 
- Рекомендуется для хранения параметров Text и ByteArray.
- Использует для хранения партицию "nvs". Но можно задать другую.
```cpp          
    // задание партиции nvs1 для хранения MWOSStorageEspNVS
    #define MWOS_NVS_PARTITION "nvs1" 
```
```cpp          
    // отключить хранилище NVS по умолчанию
...
[MWOSStorage](MWOSStorageBytes.h). 
