...
#include "storages/MWStorageBytes.h"
#include <backup.h>
 ...
 **/
class MWStorageSTM32BKPReg : public MWStorageBytes {
public:
    MWStorageSTM32BKPReg() : MWStorageBytes((char *) F("STM32_BKP_REG")) {
        MW_LOG_LN(F(" Inited!"));
    }
    ...
    virtual uint8_t read(size_t byteOffset) {
        ...
        MW_LOG_TIME(); MW_LOG(F("BKP reg read "));  MW_LOG(regAddr); MW_LOG('='); MW_LOG_LN(v, HEX);
        disableBackupDomain();
        return v;
    }
    ...
    virtual void write(size_t byteOffset, uint8_t v) {
        ...
        MW_LOG_TIME(); MW_LOG(F("BKP reg write "));  MW_LOG(regAddr); MW_LOG('='); MW_LOG_LN(regData,HEX);
        disableBackupDomain();
    }
