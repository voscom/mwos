 ...
 *
 */
class MWStorageEsp8266RTC : public MWStorageBytes {
public:
    MWStorageEsp8266RTC() : MWStorageBytes((char *) F("ESP8266_RTC")) {
        MW_LOG_LN(F("ESP8266 RTC RAM"));
    }
    ...
    virtual uint8_t read(size_t byteOffset) {
        ...
        if (ESP.rtcUserMemoryRead(byteOffset & 0b11111111111111111111111111111100, (uint32_t *)&regData, 4 )) {
            ...
            MW_LOG_TIME(); MW_LOG(F("ESP RTC read "));  MW_LOG(byteOffset); MW_LOG('='); MW_LOG_LN(v, HEX);
        } else {
            MW_LOG_TIME(); MW_LOG(F("ESP RTC read error: "));  MW_LOG_LN(byteOffset);
        }
        return v;
    }
    ...
    virtual void write(size_t byteOffset, uint8_t v) {
        ...
        if (ESP.rtcUserMemoryRead(byteOffset & 0b11111111111111111111111111111100, (uint32_t *)&regData, 4 )) {
            ...
            if (ESP.rtcUserMemoryWrite(byteOffset & 0b11111111111111111111111111111100, (uint32_t *)&regData, 4)) {
                MW_LOG_TIME(); MW_LOG(F("ESP RTC write "));  MW_LOG(byteOffset); MW_LOG('='); MW_LOG_LN(v, HEX);
            } else {
                MW_LOG_TIME(); MW_LOG(F("ESP RTC write error: "));  MW_LOG_LN(byteOffset);
            }
        } else {
            MW_LOG_TIME(); MW_LOG(F("ESP RTC read error: "));  MW_LOG_LN(byteOffset);
        }
