...
#if defined(ESP32)
#include "hardware/platforms/canbus_esp32.h"
...
//#define MWOS_SEND_BUFFER_USE 1 // признак, что надо использовать кольцевой буфер отправки

/***
 * Работа с шиной CAN
 * может работать с CAN-сообщениями
 * а так же в режиме Stream (с буффером 8 байт)
 * Можно импользовать с MWOSNetReceiver при включенном кольцевом буффере отправки MWOS_SEND_BUFFER_USE
 */
...
public:
    ...
    /**
     * Прочитать сообщение из буфера принятых сообщений
