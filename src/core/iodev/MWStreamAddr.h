...
//
#ifndef MWOS3_MWStreamAddr_H
#define MWOS3_MWStreamAddr_H
 ...
 *
 */
class MWStreamAddr : public Stream {
public:
    ...
    size_t readBuffer(uint16_t address, void * buffer, uint16_t size) {
        addr=address;
        return readBytes((char *) buffer, size);
    }

    size_t writeBuffer(uint16_t address, void * buffer, uint16_t size) {
        addr=address;
