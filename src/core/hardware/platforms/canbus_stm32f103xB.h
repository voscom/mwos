...
*/
typedef enum {PA11_PA12 = 0, PA8_PA9=2, PD0_PD1=3} CAN_REMAP;
...
typedef struct
{
    ...
    uint8_t  data_length_code;       /* Length of data field in bytes                   */
                          ...
                          CAN_bit_timing_config_t* const out_timings)
{
    ...
    out_timings->time_segment_2 = bs2;
        ...
        MW_LOG(F("peripheral_clock_rate=")); MW_LOG_LN(peripheral_clock_rate);
        MW_LOG(F("timings.baud_rate_prescaler=")); MW_LOG_LN(out_timings->baud_rate_prescaler);
        MW_LOG(F("timings.time_segment_1=")); MW_LOG_LN(out_timings->time_segment_1);
        MW_LOG(F("timings.time_segment_2=")); MW_LOG_LN(out_timings->time_segment_2);
        MW_LOG(F("timings.resynchronization_jump_width=")); MW_LOG_LN(out_timings->resynchronization_jump_width);
    return 0;
}
...
/***
 * Настроить шину CAN
 * @param pinRx Порт RX может быть PA11,PA8,PD0
 * @param pinTx Порт TX может быть PA12,PA9,PD1
...
bool CAN_Init(uint8_t pinRx, uint8_t pinTx, uint16_t speedKBPS=250, int32_t filter_id=-1) {
    ...
    if (pinRx == PA11 && pinTx == PA12) {
        ...
        // 0b1011
        //   MODE=11(Output mode, max decelerateSpeed 50 MHz)
    ...
    if (result) {
        MW_LOG_LN(F("ComputeCANTimings result=1"));
        return false;
    }
    ...
    // Two 32-bit registers of filter bank x are in Identifier List mode
    // Filter assigned to FIFO 0
    // Filter bank register to all 0
    CANSetFilter(0, 1, 1, 0, filter_id, 0x0UL);
    ...
    if (can1) {
        MW_LOG_LN(F("CAN1 initialize ok"));
    } else {
        MW_LOG_LN(F("CAN1 initialize fail!!"));
        return false;
    }
    return true;
}
...
void CANSend(CAN_msg_t* CAN_tx_msg) {
    ...
    CAN1->sTxMailBox[0].TDTR |= CAN_tx_msg->data_length_code & 0xFUL;
...
bool isTransmitBusyCAN() {
    return CAN1->sTxMailBox[0].TIR & 0x1UL;
}
